        ...
        <DialogContent className="bg-slate-800 text-white border-slate-700 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-green-400 flex items-center gap-2">
              ...
              Hasil Registrasi ONT
            </DialogTitle>
            <DialogDescription className="text-slate-400">
              ONT berhasil didaftarkan! Berikut detail informasinya:
            </DialogDescription>
          </DialogHeader>
          ...
          {registrationResult && (
            <div className="space-y-4">
              ...
              <div className="bg-slate-900/50 rounded-lg p-4 space-y-2">
                <h3 className="font-semibold text-blue-300 mb-3">📋 Detail ONT</h3>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div className="text-slate-400">Serial Number:</div>
                  <div className="text-white font-mono">{registrationResult.serial_number}</div>
                  
                  <div className="text-slate-400">ONT ID:</div>
                  <div className="text-white font-semibold">{registrationResult.ont_id}</div>
                  
                  <div className="text-slate-400">Frame/Slot/Port:</div>
                  <div className="text-white">
                    {registrationResult.frame}/{registrationResult.board}/{registrationResult.port}
                  </div>
                  
                  <div className="text-slate-400">VLAN:</div>
                  <div className="text-white">{registrationResult.vlan}</div>
                  
                  {registrationResult.description && (
                    <>
                      <div className="text-slate-400">Description:</div>
                      <div className="text-white">{registrationResult.description}</div>
                    </>
                  )}
                </div>
              </div>
              ...
              {/* Optical Power Info */}
              {registrationResult.optical_info && (
                <div className="bg-gradient-to-r from-blue-900/50 to-green-900/50 rounded-lg p-4 border border-blue-700/50">
                  <h3 className="font-semibold text-green-300 mb-3 flex items-center gap-2">
                    📡 Signal Strength (Optical Power)
                  </h3>
                  
                  {registrationResult.optical_info.rx_power ? (
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-slate-300">Rx Optical Power:</span>
                        <span className="text-2xl font-bold text-green-400">
                          {registrationResult.optical_info.rx_power} dBm
                        </span>
                      </div>
                      
                      {/* Signal Quality Indicator */}
                      <div className="mt-3">
                        {parseFloat(registrationResult.optical_info.rx_power) >= -20 && parseFloat(registrationResult.optical_info.rx_power) <= -15 ? (
                          <div className="bg-green-500/20 border border-green-500 rounded p-2 text-center">
                            <p className="text-green-400 font-semibold">✅ Signal Bagus (Excellent)</p>
                            <p className="text-xs text-green-300">Kualitas sinyal sangat baik</p>
                          </div>
                        ) : parseFloat(registrationResult.optical_info.rx_power) < -20 && parseFloat(registrationResult.optical_info.rx_power) >= -25 ? (
                          <div className="bg-yellow-500/20 border border-yellow-500 rounded p-2 text-center">
                            <p className="text-yellow-400 font-semibold">⚠️ Signal Cukup (Good)</p>
                            <p className="text-xs text-yellow-300">Kualitas sinyal baik</p>
                          </div>
                        ) : (
                          <div className="bg-red-500/20 border border-red-500 rounded p-2 text-center">
                            <p className="text-red-400 font-semibold">❌ Signal Lemah (Weak)</p>
                            <p className="text-xs text-red-300">Periksa kabel fiber atau koneksi</p>
                          </div>
                        )}
                      </div>
                      
                      <div className="text-xs text-slate-400 mt-2">
                        <p>Range ideal: -15 dBm sampai -25 dBm</p>
                      </div>
                    </div>
                  ) : (
                    <div className="text-slate-400 text-center py-2">
                      <p>⚠️ Data optical power tidak tersedia</p>
                      <p className="text-xs mt-1">Device mungkin offline atau command gagal</p>
                    </div>
                  )}
                </div>
              )}
